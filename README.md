...
[![Latest Release](https://img.shields.io/github/release/cabrera-evil/semantic-release-expo/all.svg?style=flat-square)](https://github.com/cabrera-evil/semantic-release-expo/releases)
...
{
    "verifyConditions": [
        "@cabrera-evil/semantic-release-expo",
...
Unfortunately, all apps are different and sometimes require a specific release flow.
...
For example, if you need to [create multiple versions/flavours and allow them to work side-by-side][info-multiple-manifests], you need multiple manifests.
...
```json
{
    "prepare": [
        {
            "path": "@cabrera-evil/semantic-release-expo",
            "manifests": [
                "app.test.json",
                "app.staging.json",
                "app.production.json"
...
For example, iOS can simply use the exact semantic version (e.g. `2.5.1`) but [Android can't][info-android-semver].
To allow multiple "tactics" or personal favorites, you can change the so called "versioning templates".
These templates use [lodash template][lodash-template] to build new versions.
...
[repo-issue-increments]: https://github.com/cabrera-evil/semantic-release-expo/issues/4#issuecomment-417583573
